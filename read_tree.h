...
float trkpt[29999];       // track pT
float trketa[29999];      // track eta
float trkphi[29999];      // track phi
float trkpterr[29999];    // track pT error (uncertainty)
float trkdcaxy[29999];    // track dxy impact parameter (transverse distance between primary vertex and collision - distance of closest approuch - DCA)
float trkdcaz[29999];     // track dz impact parameter (longitudinal distance between primary vertex and collision - distance of closest approuch - DCA)
float trkdcaxyerr[29999]; // track dxy error (uncertainty)
float trkdcazerr[29999];  // track dxy error (uncertainty)
float trkchi2[29999];     // track reconstruction chi2 of the fitting
float pfEcal[29999];      // particle flow energy deposit in ECAL
float pfHcal[29999];      // particle flow energy deposit in HCAL
float trkmva[29999];      // track mva for each step
int trkalgo[29999];       // track algorithm/step
unsigned char trkndof[29999];       // track number of degrees of freedom in the fitting 
int trkcharge[29999];     // track charge
unsigned char trknhits[29999];      // number of hits in the tracker
unsigned char trknlayer[29999];     // number of layers with measurement in the tracker
unsigned char trkpixhits[29999];// number of hits in the pixel detector
bool highpur[29999];      // tracker steps MVA selection
